...
@section('title')
    {{ __('Create Receipt Transaction') }} | {{ config('app.name') }}
@endsection
...
@section('breadcrumb')
    ...
    <span class="kt-subheader__breadcrumbs-separator"></span>
    <a href="{{ route('receipt.create') }}" class="kt-subheader__breadcrumbs-link">{{ __('Create') }}
        {{ __('Receipt Transaction') }}</a>
@endsection

@section('content')
    <form class="kt-form" method="POST" action="{{ route('receipt.store') }}">
        @csrf
        <div class="kt-portlet" id="kt_page_portlet">
            <div class="kt-portlet__head kt-portlet__head--lg">
                <div class="kt-portlet__head-label">
                    <h3 class="kt-portlet__head-title">Create Medical Record</h3>
                </div>
            ...
            <div class="kt-portlet__body">
                ...
                @if (!request()->is('receipt/create/medicine*'))
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-3">
                                    ...
                                    <input type="text" id="insurance_name" class="form-control" disabled>
                                </div>
                                <div class="col-lg-3">
                                    <label>{{ __('Insurance No') }}</label>
                                    <input type="text" id="insurance_no" class="form-control" disabled>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table" id="service-table">
                                    ...
                                    <tbody id="service-table-body">
                                        <tr class="service-row">
                                            <td>
                                                <select name="services[0][service_id]" class="form-control service-select">
                                            ...
                                            <td>
                                                <select name="services[0][unit_id]" class="form-control kt_selectpicker"
                                            ...
                                            <td>
                                                <select name="services[0][doctor_id]" class="form-control kt_selectpicker"
                                                    data-live-search="true"
                                            ...
                                            <td><input name="services[0][price]" type="number"
                                                    class="form-control service-price format-number" readonly></td>
                                        </tr>
                                    </tbody>
                                </table>
                            ...
                            <table class="table" id="medicine-table">
                                ...
                                <tbody id="medicine-table-body">
                                    <tr class="medicine-row">
                                        <td>
                                            <select name="medicines[0][item_id]" class="form-control medicine-select">
                                                <option value="">{{ __('Select medicine option') }}</option>
                                                @foreach ($medicines as $medicine)
                                                    <option value="{{ $medicine->id }}"
                                                        data-price="{{ $medicine->price }}"
                                                        data-unit="{{ $medicine->unit }}">{{ $medicine->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td><input type="number" name="medicines[0][item_qty]"
                                                class="form-control medicine-quantity" min="1" value="1">
                                        </td>
                                        <td><input type="text" name="medicines[0][item_unit]"
                                                class="form-control medicine-unit" readonly></td>
                                        <td><input type="number" name="medicines[0][unit_price]"
                                                class="form-control medicine-price format-number" readonly></td>
                                        <td><input type="number" name="medicines[0][total_price]"
                                                class="form-control medicine-total-price format-number" readonly></td>
                                    </tr>
                                </tbody>
                            </table>
                            <button type="button" class="btn btn-dark ml-3"
                        ...
                        <div class="form-group row">
                            @if (!request()->is('receipt/create/medicine*'))
                                <div class="col-lg-4">
                                    ...
                                    <input type="number" id="services-total" class="form-control format-number" readonly>
                                </div>
                            @endif
                            <div class="col-lg-4">
                                <label>{{ __('Total Amount for Medicines') }}</label>
                                <input type="number" id="medicines-total" class="form-control format-number" readonly>
                            </div>
                            <div class="col-lg-4">
                                <label>{{ __('Total Amount for Receipt') }}</label>
                                <input type="number" id="grand-total" class="form-control format-number" readonly>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            // Update patient information based on selected patient
            $('select[name="patient_id"]').change(function() {
                var selectedOption = $(this).find('option:selected');
                $('#nik').val(selectedOption.data('nik'));
    ...
    <script>
        $(document).ready(function() {
            let serviceIndex = 1; // To track the index of the dynamic rows
    ...
    <script>
        $(document).ready(function() {
            ...
            // Initialize the first row
            updateMedicineRow($('#medicine-table-body .medicine-row').first());
        });
    </script>
@endsection
